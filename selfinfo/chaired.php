...
<body>
  ...
  <div class="overlay container">
    <table class="table">
      ...
      <h1><? if ($_SESSION['nimbus_access'] > 0) { echo $_SESSION['nimbus_first_name'] . "'s "; } else { echo "My "; } ?>Chaired Events</h1>
      <p>Filter by: <a href="/~circlek/selfinfo/events.php">All Events</a>, <a href="/~circlek/selfinfo/eventsawards.php">Award Categories</a>, <a href="/~circlek/selfinfo/committee.php">Committee Meetings</a>, <a href="/~circlek/selfinfo/kiwanis.php">Kiwanis Meetings</a></p>
      <tbody>
      <? foreach ($events as $event) { 
        ...
        <tr>
          ...
          <td>
            <? if($event['chair_id'] != $_GET['id']) {?>
            <a onClick="javascript:usermodalOpen(<?= $event['event_id'] ?>);"> <? echo $event['name']; ?> </a>
            <? } else { ?>
            <a onClick="javascript:usermodalOpen(<?= $event['event_id'] ?>);"> <? echo "CHAIR!!!!", $event['name']; ?> </a>
            <? } ?>
